    ...
    return new THREE.ShaderMaterial({
      ...
      void main() {
        ...
        if (isHovered > 0.5) {
          ...
          float wavePhase = pos.x * 5.0 + time * 6.0; // Faster frequency
          float waveAmplitude = sin(wavePhase) * 0.15; // Increased amplitude
          ...
          float secondaryWave = sin(pos.x * 10.0 + time * 15.0) * 0.05 * dampening;
...
function GalleryScene({
    ...
    scrollProgress
}: Omit<InfiniteGalleryProps, 'className' | 'style'> & { scrollProgress: number }) {
    ...
    useFrame((state, delta) => {
        ...
        const velocity = (scrollProgress - lastScrollProgress.current) * 100;
        ...
        // Drive Z position by scrollProgress
        // We want the gallery to move forward as we scroll down
        const scrollZ = scrollProgress * totalRange * 10; // Multiplier determines speed relative to scroll

        planesData.current.forEach((plane, i) => {
            // Initial Z + scroll offset
            const baseZ = ((depthRange / Math.max(visibleCount, 1)) * i);
            ...
            if (wrapsBackward > 0 && imageAdvance > 0 && totalImages > 0) {
                // Simplified index calculation for deterministic scroll
                plane.imageIndex = (i - wrapsBackward * imageAdvance) % totalImages;
                if (plane.imageIndex < 0) plane.imageIndex += totalImages;
            }

            // For deterministic scroll, we can just calculate index based on total scroll
            const totalScrollWraps = Math.floor((baseZ + scrollZ) / totalRange);
            const effectiveIndex = (i + totalScrollWraps * imageAdvance) % totalImages;
            plane.imageIndex = effectiveIndex >= 0 ? effectiveIndex : effectiveIndex + totalImages;
...
export default function InfiniteGallery({
    ...
    const smoothProgress = useSpring(scrollYProgress, {
        mass: 0.5, // Increased mass for more inertia
        stiffness: 50, // Reduced stiffness for looser feel
        damping: 15, // Reduced damping for longer glide
        restDelta: 0.0001
    });
    ...
    useEffect(() => {
        // Listen to the smoothed value instead of raw scrollYProgress
